...
def analyze_case_data(
    ...
    Dùng để lấy dữ liệu các vụ án, vụ việc và phân tích tùy theo yêu cầu của người dùng, tạo ra các câu lệnh sql phù hợp để lấy kết quả người dùng mong muốn
     Usage Guidelines for AI:
        ...
        2. Viết query phù hợp với yêu cầu, nếu tìm kiếm chính xác không cho ra kết quả thì có thể dùng like
        3. Validate kết quả có hợp lý không
...
def analyze_email_data(
    ...
    """
        Usage Guidelines for AI:
        1. Chỉ cần hiển thị kết quả cuối cùng
        2. Viết query phù hợp với yêu cầu, đối với tìm kiếm, nếu tìm kiếm chính xác không cho ra kết quả thì có thể dùng like
        3. Validate kết quả có hợp lý không
    ...
    # Xử lý từng dòng dữ liệu, format ngày tháng
    rows = []
    for row in output_records:
        formatted_row = []
        for h in headers:
...
@mcp.tool()
def get_list_sheet_name() -> str:
    """
    Lấy tên các sheet của file dữ liệu và trả về dạng Markdown
    """
...
def get_current_time() -> str:
    """
    Lấy thời gian hiện tại của hệ thống theo định dạng HH:mm:SS dd-MM-yyyy.
...
def get_current_time_with_timezone(timezone: str = "UTC") -> str:
    """
    Lấy thời gian hiện tại với múi giờ cụ thể, format HH:mm:SS dd-MM-yyyy.
